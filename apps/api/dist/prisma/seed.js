...
async function main() {
    ...
    const admin = await prisma.user.upsert({
        ...
        update: { passwordHash, role: client_1.Role.ADMIN, name: 'Ada Admin' },
        create: {
    ...
    const [manager1, manager2] = await Promise.all([
        prisma.user.upsert({
            ...
            update: { passwordHash, role: client_1.Role.MANAGER, name: 'Maya Manager' },
            create: {
        ...
        prisma.user.upsert({
            ...
            update: { passwordHash, role: client_1.Role.MANAGER, name: 'Mark Manager' },
            create: {
    ...
    const customers = await Promise.all(['carol', 'chad', 'casey'].map((slug, i) => prisma.user.upsert({
    ...
    const generated = Array.from({ length: 76 }, (_, i) => {
    ...
    const hotelData = [...namedHotels, ...generated];
 ...
 */
async function upsertHotelByName(h) {
    ...
    const data = {
        ...
        stars: h.stars,
        status: client_1.HotelStatus.ACTIVE,
...
async function ensureRoomTypesAndRooms(hotel) {
    const types = [
 ...
 */
async function ensureSampleBooking(customer) {
    if (!customer)
        return;
