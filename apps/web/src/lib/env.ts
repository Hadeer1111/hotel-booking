 ...
 * - Otherwise in production we use same-origin `/v1`, proxied by `next.config.mjs`
 *   rewrites to `API_BACKEND_ORIGIN/v1/...` — no rebuild to change the backend.
 */
...
// the literal "/v1" unless NEXT_PUBLIC_API_URL is set (see top-of-file note).
const apiUrl =
  explicitPublic ??
...
if (
  ...
  console.warn(
    '[hotel-booking] NEXT_PUBLIC_API_URL targets localhost in a production build. Remove it to use same-origin /v1 proxy with API_BACKEND_ORIGIN, or set NEXT_PUBLIC_API_URL to your public API (…/v1) and redeploy.',
  );
}
